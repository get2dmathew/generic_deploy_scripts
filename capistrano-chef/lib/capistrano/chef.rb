...
module Capistrano::Chef
   ...
   def self.load_into(configuration)
	 ...
	 # set artifactory credentials if we need to get artifact from Artifactory
         def set_artifactory_from_data_bag(data_bag = :apps, secret = nil)
            raise ':application must be set' if fetch(:application).nil?
            artifactory_secret = Chef::EncryptedDataBagItem.load_secret("/var/lib/jenkins/.chef/encrypted_data_bag_secret")
            capistrano_chef.get_encrypted_data_bag_item("artifactory", data_bag, artifactory_secret).each do |k, v|
		set k, v
            end
         end
